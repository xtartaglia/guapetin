  ...
  useEffect(()=>{
    ...
    function handleClick() {
      ...
      if (engine.gravity.y === 0) {
        ...
        qr = Composite.create()
        var sprite = createImage(domande["domanda0"].d.toUpperCase(),render.options.width/2,render.options.height/8,type)
        ...
        var update = setInterval(()=>{
          ...
          if (lastChild.position.x-player.position.x < render.options.width/4) {
            ...
            if (lastChild.position.x<-100) {
              ...
              Composite.add(engine.world, [domanda])
              sprite = createImage(domande["domanda"+j].d.toUpperCase(),render.options.width/2,(window.innerHeight/50*49)/4.5*0.4,type)
