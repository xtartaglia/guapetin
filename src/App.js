  ...
  useEffect(() => {
    ...
    function handleClick() {
        ...
        var update = setInterval(() => {
          ...
          if (lastChild.position.x - player.position.x < window.screen.width / 3) {
            ...
            if (lastChild.position.x < -100) {
              ...
              domanda = Domanda({ screenHeight: window.screen.height, groundHeight: window.screen.height / 12.5, x: player.position.x + 2500, q: domande["domanda" + j], scale: scale })
              Composite.add(engine.world, [domanda])
      ...
      if (!gameOver) {
        ...
        Body.applyForce(player, { x: player.position.x, y: player.position.y }, { x: 0, y: -0.2 })
      }
    }
