  ...
  useEffect(() => {
    ...
    function handleClick() {
      if (justStarted && !gameOver && fullscreen) {
        ...
        engine.gravity.y = 0.2 * scale * scaleX
        domanda = Domanda({ screenHeight: window.screen.height, groundHeight: window.screen.height / 12.5, x: player.position.x + 2000 * scaleX, q: domande["domanda0"], scale: scale })
        ...
        var update = setInterval(() => {
          ...
          if (lastChild.position.x - player.position.x < window.screen.width / 3) {
            ...
            if (lastChild.position.x < -100) {
              ...
              domanda = Domanda({ screenHeight: window.screen.height, groundHeight: window.screen.height / 12.5, x: player.position.x + 2000 * scaleX, q: domande["domanda" + j], scale: scale })
              Composite.add(engine.world, [domanda])
